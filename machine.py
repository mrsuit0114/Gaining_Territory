...
def alpha_beta_pruning(node, alpha, beta):
    if node.depth == DEPTH_LIMIT or len(node.config.available) < AVAIL_LIMIT:
    ...
    if node.config.isMax:
        ...
        for child in node.children:
            ...
            value = max(value, alpha_beta_pruning(child, alpha, beta))
        ...
        for child in node.children:
            child.depth = node.depth + 1 
            value = min(value, alpha_beta_pruning(child, alpha, beta))
            beta = min(beta, value)
            if beta <= alpha:
                break  # 알파 가지치기
        return value
    ...
    def find_best_selection(self):      
        ...
        if len(available) < AVAIL_LIMIT:
            ...
            for child in root.children:
                value = alpha_beta_pruning(child, float('-inf'), float('inf'))
                if value > best_value:
                    best_value = value
                    best_move = child.config.line
                return best_move
